    ...
    def create_video(self, request):
        ...
        try:
            if userData["role"] == self.config["roles"]["administrator"]:
                ...
                except: return BusinessResponse(self.random.CreateRandomId(), "ERROR", [], BusinessError(self.random.CreateRandomId(), "MISSING_FILE")).toJson()
                ...
                try: 
                    ...
                    try: free = 0 if request.form["free"] == "false" else 1
                    except: free = 1
                    try:public = 0 if request.form["public"] == "false" else 1
                    except: public = 1
                    try: trailer = 0 if request.form["trailer"] == "false" else 1
                    except: trailer = 0
                    id = self.random.CreateRandomId()
                    self.logger.log("INFO", f"Pulling data from request for new file with id: {id}")
                except Exception as e: return BusinessResponse(self.random.CreateRandomId(), "ERROR", [], BusinessError(self.random.CreateRandomId(), "CAN_NOT_READ_VIDEO_INFO")).toJson()
                try:
                    ...
                    return BusinessResponse(self.random.CreateRandomId(), "ERROR", [], BusinessError(self.random.CreateRandomId(), "CAN_NOT_SAVE_VIDEO")).toJson() 
            ...
            return BusinessResponse(self.random.CreateRandomId(), "ERROR", [], BusinessError(self.random.CreateRandomId(), "CAN_NOT_CREATE_VIDEO")).toJson()
        except Exception as e:
            self.logger.log("ERROR", str(e))
            return BusinessResponse(self.random.CreateRandomId(), "ERROR", [], BusinessError(self.random.CreateRandomId(), "CAN_NOT_CREATE_VIDEO")).toJson()